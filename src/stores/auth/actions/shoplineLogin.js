...
export const shoplineLogin = {
  async shoplineLogin(platform, storeName) {
    ...
    if(data.success && typeof data.redirect_url !== 'undefined') {
      ...
      if(data.is_onboarding) {
        redirectURL += import.meta.env.VITE_SHOPLINE_RESPONSE_REDIRECT_URL;
      }
      ...
      if (window.sessionStorage.getItem('ID_TOKEN_KEY')) {
        ...
        if(response?.stores[0]?.type === 'none') {
          router.replace({ name: routes.SHOPLINE_SELECT_STORE_TYPE});
        } else {
